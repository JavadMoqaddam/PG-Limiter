...
def _build_ip_details(
    ...
    # Device counting never includes node_id: several nodes can serve the same
    # core config, so one client is registered on all of them at once and would
    # otherwise be counted once per node.
    #   count_mode "device" -> the inbound stays in the key, so one IP reaching
    #                          two inbounds counts as two devices. This is what
    #                          detects several people sharing one connection.
    #   count_mode "ip"     -> the inbound is dropped as well: one client IP is
    #                          exactly one device.
    ...
    for conn in original_user.device_info.connections:
        ...
        if conn.node_id in cdn_nodes:
            ...
            # Pure IP counting: one client IP is one device, whatever it connects
            # to. Subnet grouping still applies when enabled (same /24, or same
            # /16+ISP, collapses to one device).
            if subnet_ip_grouping:
                if getattr(device_config, "subnet_grouping_mode", "/24") == "/16":
                    ip_isp = get_isp_name(conn.ip)
                    if ip_isp:
                        unique_devices.add(("SUBNET_ISP_GROUP", get_wide_subnet_key(conn.ip), ip_isp))
                    else:
                        unique_devices.add(("SUBNET_GROUP", get_subnet_key(conn.ip)))
                else:
                    unique_devices.add(("SUBNET_GROUP", get_subnet_key(conn.ip)))
            else:
                unique_devices.add(("IP_ONLY", conn.ip))
        elif apply_high_trust_grouping:
            # High Trust mode: IPs reaching the same inbound are one device
            # (for users who have built trust - detecting WiFi/Mobile switching)
            unique_devices.add(("HIGH_TRUST", conn.inbound_protocol))
        elif subnet_ip_grouping:
            # Subnet IP Grouping mode:
            # - Mode "/16": Wide /16 subnet (192.168.x.x) + ISP + inbound (if ISP info available)
            # - Mode "/24": Standard /24 subnet (192.168.1.x) + inbound (recommended)
            if getattr(device_config, "subnet_grouping_mode", "/24") == "/16":
                ip_isp = get_isp_name(conn.ip)
                if ip_isp:
                    wide_subnet_key = get_wide_subnet_key(conn.ip)
                    unique_devices.add(("SUBNET_ISP_GROUP", wide_subnet_key, ip_isp, conn.inbound_protocol))
                else:
                    subnet_key = get_subnet_key(conn.ip)
                    unique_devices.add(("SUBNET_GROUP", subnet_key, conn.inbound_protocol))
            else:
                subnet_key = get_subnet_key(conn.ip)
                unique_devices.add(("SUBNET_GROUP", subnet_key, conn.inbound_protocol))
        else:
